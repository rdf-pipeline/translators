...
function translate(cmumpsDiagnosisObject) {
    ...
    // These fields have cmumps sources, but don't seem to have a map to a sd DiagnosticReport.
    ...
    return fdt.clean({
        ...
        identifier : [ fdt.sdIdentifierList(get('$.type')) ], // TODO: Array of Array?
        status: 'final', // R!  registered | partial | final | corrected | appended | cancelled | entered-in-error, http://hl7-sd.github.io/valueset-diagnostic-report-status.html
        category: fdt.sdDiagnosticReportCategory(get('$.diagnosis-100417')), // Service category
        code: fdt.sdCodeableConcept(get('$.diagnosis-100417')), // R!  Name/Code for this diagnostic report
        subject: fdt.sdReferencePatient(get('$.patient-100417')), // { Reference(Patient|Group|Device|Location) }, // R!  The subject of the report, usually, but not always, the patient
        ...
        effectiveDateTime: fdt.sdDate(get('$.diagnosis-100417.date_of_onset-100417.value')),
        // effectivePeriod: { Period },
        issued: get('$.date_entered-100417'), // R!  DateTime this version was released
        performer: fdt.sdPractioner(get('$.provider-100417')), // { Reference(Practitioner|Organization) }, // R!  Responsible Diagnostic Service
        ...
        codedDiagnosis: fdt.sdCodeableConcept(get('$.diagnosis-100417')), // Codes for the conclusion
