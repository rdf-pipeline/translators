...
function translate(cmumpsPrescriptionObject) {
    ...
    return fdt.clean({
        ...
        identifier: fdt.sdExternalIdentifier(get('$._id'), 'Prescription'),
        ...
        medicationCodeableConcept: fdt.sdCodeableConcept(get('$.drug-52.label')), // Reference(Medication)
        medicationReference: fdt.sdReferenceMedication(get('$.drug-52')), // Reference(Medication)
        patient: fdt.sdReferencePatient(get('$.patient-52')),
        dispenser: fdt.sdReferencePractioner(get('$.provider-52')), // Reference(Practitioner), Practitioner responsible for dispensing medication
        authorizingPrescription: [ fdt.sdReferenceMedicationOrder(get('order_pointer-52')) ], // Medication order that authorizes the dispense, TODO: cross ref?
        type: fdt.sdCodeableConcept(get('$.status-52')), // Trial fill, partial fill, emergency fill, etc., NOT FOUND
        quantity: fdt.sdQuantity(get('$.qty-52')), // { Quantity(SimpleQuantity) }, // Amount dispensed
        daysSupply: fdt.sdQuantity(get('$.days_supply-52')), // { Quantity(SimpleQuantity) }, // Amount of medication expressed as a timing amount
        whenPrepared: whenPrepared, // Dispense processing time
        whenHandedOver: whenPrepared, // When product was given out
        destination: fdt.sdReferenceLocation(get('$.outpatient_site-52')), // Where the medication was sent
        ...
        dosageInstruction: [ // Medicine administration instructions to the patient/caregiver
            {
                ...
                additionalInstructions: fdt.sdCodeableConcept(fdt.htmlEncode(get('$.expanded_sig-52'))), // E.g. "Take with food"
                timing: fdt.sdTiming(fdt.htmlEncode(get('$.sig-52')), whenPrepared), // When medication should be administered
