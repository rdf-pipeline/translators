/**
 * Translate sd objects into cmumps objects in simple cases.
 */
 ...
 * This is useful to clean up sd transformations that really shouldn't be there.
...
/**
 * Translate cmumpsDate string in format yy(yy)?-mm-dd to sd format mm-dd-yyyy.
 * @param {string} cmumpsDate - yy(yy)?-mm-dd
 * @returns {string || undefined} - mm-dd-yyyy
 */
function sdDate(cmumpsDate) {
...
/**
 * Given a 'Dosage Text', create a timing object, http://hl7-sd.github.io/datatypes.html#Timing
 ...
 * @see {http://hl7-sd.github.io/datatypes.html#Timing}
 */
function sdTiming(sig, when) {
    ...
    return clean({
        ...
        code: sdCodeableConcept(sig) // QD | QOD | Q4H | Q6H | BID | TID | QID | AM | PM +
    });
}


/**
 * Translate a cmumps name format 'LAST, FIRST MI TITLE' to sd HumanName
 * http://hl7-sd.github.io/datatypes.html#HumanName
 *
 * @param cmumpsName
 ...
 *
 */
function sdHumanName(cmumpsName) {
    if (cmumpsName === undefined) return undefined;
    // microparse the cmumps name value into sdParts 'last, first mi title'
...
/**
 * Generate a sd ContactPoint.
 * @param cmumpsContactPoint
 * @returns {*}
 * @see {link: 'http://hl7-sd.github.io/datatypes.html#ContactPoint'}
 */
function sdContactPoint(cmumpsContactPoint) {
...
/**
 * Turn a US social security number into a sd identifier.
 * @param {string} cmumpsSsn - social security number
 * @param {string} cmumpsDodId - dod id
 * @returns {Array.<sd.Address>}
 *
 ...
 * @param cmumpsDodId
 */
function sdIdentifier(cmumpsSsn, cmumpsDodId) {
    if (cmumpsSsn === undefined && cmumpsDodId === undefined) return undefined;
    // filter function, id -> sd_id
    // TODO mike@carif.io: use sdCodeableConcept instead?
    function f(id) {
        if (id) {
            return {
                use: 'usual',
...
/**
 * Create a sd Identifier and associated CodeableConcept. The reference is considered "external" to sd, e.g.
 ...
 * @see {https://hl7-sd.github.io/datatypes.html#Identifier}
 */
function sdExternalIdentifier(value, codeableConceptType) {
    if (value === undefined) return undefined;
    return {
        use: 'usual',
        type: sdCodeableConcept(codeableConceptType),
...
/**
 * Given an identifier 'id' with an optional prefix, create a list of a single sd cmumps identifier.
 * @param {string} id
 * @param {string} prefix -- optional prefix, used by turtle amoungst other things.
 * @returns {List[Identifier] || undefined}
 */
function sdIdentifierList(id, prefix) {
...
/**
 * Translate a cmumps marital status into a sd maritalStatus(http://hl7-sd.github.io/valueset-marital-status.html)
 * Note: The marital status 'U' in sd maps to both "UNKNOWN" and "UNMARRIED". These are two separate concepts.
 * Therefore, I've mapped cmumps 'UNKNOWN' to sd 'U'.
...
// TODO mike@carif.io: use sdCodeableConcept()
function sdMaritalStatus(cmumpssMaritalStatus) {
    if (cmumpssMaritalStatus === undefined) return undefined;
    // mapping from http://hl7-sd.github.io/v3/MaritalStatus/index.html
    ...
    if (_.contains(["DIVORCED", "SINGLE,NEVER MARRIED", "MARRIED", "LEGALLY SEPARATED", "UNKNOWN"], ms)) {
        ...
        throw new Error("'" + ms + "' not mapped to sd");
    }
}

/**
 * Create a sd address from an intermediate address using the JSONPath style. By intermediate here, I
 * "raise" the cmumps address into an object with certain keys and then convert that into a sd Address.
 * @param {object} address -- the postal address as an object
 * @returns {Address || undefined}
 * @see {https://hl7-sd.github.io/datatypes.html#address}
 */
function sdAddress(address) {
...
/**
 * Turn a cmumps code, i.e. 'cmumpss:Patient-2' or 'cmumpss:Prescription-52' into a sd CodeableConcept.
 * @param cmumpsTypeCode
 * @returns {CodeableConcept}
 */
function sdCodeableConcept(cmumpsTypeCode) {
 ...
 */
function sdCodeableConceptList(cmumpsTypeCode) {
    if (cmumpsTypeCode === undefined) return undefined;
    return [ sdCodeableConcept(cmumpsTypeCode) ];
}
...
/**
 * http://hl7-sd.github.io/valueset-diagnostic-service-sections.html
 * @param cmumps "service category"
 */
function sdDiagnosticReportCategory(cmumpsCode) {
    if (cmumpsCode === undefined) return undefined;
    // table taken from http://hl7.org/sd/v2/0074/index.html
    var fromTo = {
        '': 'AU', // Audiology
        '': 'BG', // Blood Gases
        '': 'BLB', // Blood Bank
    ...
    // TODO mike@carif.io: awaits implementing labs in sd vs shex.
    return 'OTH';
...
// out because the sd website presents them as different things. And we
// might eventually generate different stuff.


/**
 * Given a cmumpsProvider, return a reference to a sd Practioner.
 * @param {object} cmumpsProvider
 * @returns {{reference: id, display: label}}
 */
function sdReferencePractioner(cmumpsProvider) {
    if (cmumpsProvider === undefined) return undefined;
    return clean({
        // TODO carif: make sure this is correct
        // reference: 'urn:local:sd:MedicationDispense:' + cmumpsProvider.id,
...
/**
 * Given a cmumpsDrug (drug-52), return a reference to a sd Medication.
 * Usually this is documented as Reference(Medication).
 * @param {object} cmumpsDrug
 * @returns {{reference: id, display: label}}
 */
function sdReferenceMedication(cmumpsDrug) {
...
/**
 * Given a cmumpsOrder, return a reference to a sd MedicationOrder.
 * Usually this is documented as Reference(MedicationOrder).
 * @param cmumpsOrder
 * @returns {{reference: string, display: string} || undefined}
 */
function sdReferenceMedicationOrder(cmumpsOrder) {
...
/**
 * Create a sd Reference(Patient) based on the cmumps patient id.
 * @param {String} cmumpsPatientId, usually of the form /2-\d+/
 * @returns {{display: *}}
 */
function sdReferencePatient(cmumpsPatient) {
...
/**
 * Create a sd Reference(Location) based on the cmumpsLocation.
 * @param {object} cmumpsLocation
 * @returns {{reference: *, display: *} || undefined}
 */
function sdReferenceLocation(cmumpsLocation) {
...
/**
 * Create a sd Reference(Organization) based on the cmumpsOrganization.
 * @param {object} cmumpsOrganization
 * @returns {{reference: *, display: *} || undefined}
 */
function sdReferenceOrganization(cmumpsOrganization) {
    ...
    var result = clean({
        ...
        telecom: [ sdContactPoint(JSONPath({path: '$.telecom', wrap: false}, cmumpsOrganization)) ], // C? A contact detail for the organization
        address : [ sdAddress(JSONPath({path: '$.address', wrap: false}, cmumpsOrganization)) ], // C? An address for the organization
        ...
        //     purpose: { sdCodeableConcept }, // The type of contact
    ...
    if (result) {
        result.type = sdCodeableConcept('medical'), // Kind of organization
...
/**
 * Given quantity value and units, create a sd Quantity
 * @param {int} value -- the quantity
 * @param {string} units -- the units as a string, optional, default 'unsupplied'
 * @returns {{value: int, unit: string, system: 'cmumps', code: 'cmumps'} || undefined} -- a sd Quantity
 * @see {http://hl7-sd.github.io/datatypes.html#Quantity}
 */
function sdQuantity(value, units) {
    if (value === undefined) return undefined;
...
/**
 * Given a cmumps provider (provider-6), translate that into a sd practioner. Gets the full treatment including
 * warnings and participating properties.
 * @param {object} cmumpsProvider
 * @returns {Practioner || undefined}
 * @see{https://hl7-sd.github.io/practitioner.html}
 */
function sdPractioner(cmumpsProvider, options) {
    if (cmumpsProvider === undefined) return undefined;
    ...
    var sdPractioner = {
        ...
        // from Resource: id, meta, implicitRules, and language
        // from DomainResource: text, contained, extension, and modifierExtension
        identifier: fetch1('$.type', function(i) { return [sdIdentifier(i)]; }), // A identifier for the person as this agent
        active: true, // Whether this practitioner's record is in active use
        name: fetch1('$.id.label', sdHumanName), // A name associated with the person
    ...
    if (options.participants) addParticipants(sdPractioner, participatingProperties);
    if (options.warnings) addWarnings(sdPractioner, warnings);
    clean(sdPractioner);
    // Additional semantic processing here
    return sdPractioner;
}

// Helper functions specific to a cmumps Patient translation with no "handle" in the sd spec (e.g. HunmanName).
...
/**
 * Convert bd.value to a sd Date
 * @param bd
 * @returns {string}
 */
function sdPatientBirthDate(bd) {
    if (bd === undefined) return undefined;
    if (_.has(bd, 'value')) return sdDate(bd.value);
}
...
function sdId(sdType, id) {
    if (sdType === undefined || id === undefined) return undefined;
    return 'urn:local:sd:' + sdType + ':' + id;
}

/**
 * Create LAST[, FIRST] from cd last and first name.
 * @param cdLast
 * @param cdFirst
 * @returns {string LAST, FIRST}
 */
function cmumpsPatientLabel(cdLast, cdFirst) {
    if (! cdLast) return undefined;
    var result = cdLast.toUpperCase();
    if (cdFirst) {
        result += ', ' + cdFirst;
    }
    return result;
...
/**
 * Converts an sdDate into a cmumps Date mm/dd/yyyy -> yyyy-mm-dd
 * @param {string mm/dd/yyyy} sdDate
 * @returns {string yyyy-mm-dd}
 * TODO carif: http://momentjs.com/, it's nutty to be parsing and transforming dates.
 */
function cmumpsDate(sdDate) {
    if (! sdDate) return undefined;
    var result = sdDate.match(/^(\d{2})\/(\s{2})\/(\d{4})/);
    if (result) {
...
[makeJsonFetcher1, makeGetter, peek, eat, clean, required, sdDate, sdHumanName, sdContactPoint,
    sdIdentifier, sdIdentifierList, sdMaritalStatus, sdAddress, tbs,
    sdReferenceLocation, sdReferencePatient, sdReferencePractioner,
    sdDiagnosticReportCategory, sdCodeableConcept, sdCodeableConceptList,
    sdReferenceMedication, sdReferenceMedicationOrder, sdQuantity, sdReferencePatient,
    sdTiming, sdReferenceLocation, sdReferenceOrganization, sdExternalIdentifier, sdPractioner,
    sdPatientGender, sdPatientBirthDate, sdPatientState, sdPatientCountry, sdId,
    htmlEncode, cmumpsPatientLabel
