/**
 * Manipulate the sd json object in understandable ways.
 * TODO mike@carif.io: can this module be generated from the sd artifacts in some way?
 */
...
/**
 * Generate "now" as a timestamp parsable by sd. There are several acceptible formats.
...
/**
 * Extract the *single* patient from the sdObject using JSONPath.
 * @param {object} sdObject
 * @returns {Patient}
 */

function extractPatient(sdObject) {
    return JSONPath({json: sdObject, path: '$.entry[0].resource[?(@.resourceType=="Patient")]'})[0];
}
 ...
 * @returns {Patient}
 */
function extractDemographics(sdObject) {
    return [ extractPatient(sdObject) ];
}

/**
 * Extract all medications from the sdObject using JSONPath.
 * @param {object} sdObject
 * @returns {Array[MedicationDispense]}
 */
function extractMedications(sdObject) {
    return JSONPath({json: sdObject, path: '$.entry[0].resource[?(@.resourceType=="MedicationDispense")]'});
}

/**
 * Extract all labs from the sdObject using JSONPath.
 * @param sdObject
 * @returns {Array[Observation]}
 */
function extractLabs(sdObject) {
    return JSONPath({json: sdObject, path: '$.entry[0].resource[?(@.resourceType=="DiagnosticObservation")]'});
}

/**
 * Extract all Diagnoses from the sdObject using JSONPath.
 * @param sdObject
 * @returns {Array[DiagosticReport]}
 */
function extractDiagnoses(sdObject) {
    return JSONPath({json: sdObject, path: '$.entry[0].resource[?(@.resourceType=="DiagnosticReport")]'});
}

/**
 * Extract all Procedures from the sdObject using JSONPath.
 * @param {object} sdObject
 * @returns {Array[Procedure]}
 */
function extractProcedures(sdObject) {
    return JSONPath({json: sdObject, path: '$.entry[0].resource[?(@.resourceType=="Procedure")]'});
}
 ...
 */
function addWarnings(sdResult, warnings) {
    ...
    if (warnings.length > 0) {
        ...
        if (sdResult.extension) {
            // If there are already some extensions, add this to the list.
            sdResult.extension.push({url: 'cmumps', extension: warningsAsExtensions});
        } else {
...
// This is redundant, but can be useful if you come to this API with prior knowledge of the sd resourceTypes.
// Note that _.keys(module.exports.resourceTypes) will enumerate the "covered" sd resourceTypes.
...
// reverse the resourceTypes mapping, should report the sd return type for an extractor as a string, e.g.
// var sd = require('sd); sd.returns['extractMedications'] => 'MedicationDispense'
// Useful for mocha tests.
_.pairs(module.exports.resourceTypes).forEach(function(p) { module.exports.returns[p[1]] = p[0]; });


// parts, the parts of sd knows how to extract
