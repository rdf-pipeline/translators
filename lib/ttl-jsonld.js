...
function rdfToJsonLd(input, frame, filterBNodeAttrs) {
    ...
    return buildJSON(input).then(function(json) {
        if (frame) {
            ...
            var roots = getRoots(input);

            // stash the original @id (blank node) if we have those (ShEx path)
            if (!_.isEmpty(roots)) { 
                _.map(json, function(graph) { 
                      _.each(roots, function(root) {
                           if (graph['@id'] === root) { 
                               // What  prefix?  Discuss with David
                               // graph['http://hokukahu.com/schema/cmumpss#bnode'] = graph['@id'];
                               graph['http://www.w3.org/2000/01/rdf-schema#bnode'] = graph['@id'];
                           }
                      });
                });
            }
            ...
            return new Promise(function(resolve) { 
                resolve(jsonld.frame(json, frame, {embed: '@always'}));

            }).then(framedJson => { 

                // If we have blank node roots (normal for ShEx paths), then we need
                // to filter down to just the roots
                if (!_.isEmpty(roots)) {
                    // Filter the framed JSON-LD to retain only the real root graph elements
                    var graphs = _.filter(framedJson['@graph'], 
                        function(graph) { 
                            if (_.isUndefined(graph['rdfs:bnode']) || _.isEmpty(roots)) return false;
                            return _.reduce(roots, function(memo, root) {
                                if (memo == true) return memo;
                                var keeper = (graph['rdfs:bnode'] == root);  // keep only graphs with a root id
                                if (keeper) delete graph['rdfs:bnode'];
                                return keeper || memo;
                            }, false);
                    });  

                    framedJson['@graph'] = _.map(graphs, function(graph) {
                       return filterBNodes(graph, filterBNodeAttrs);
                    });
                } else {
                    framedJson['@graph'] = filterBNodes(framedJson['@graph'], filterBNodeAttrs);
                }
...
/**
 * Get the root graph elements.
 *
 * @param input RDF JS Interface Graph object
 *
 * @return an array of root graph IDs.  These roots are typically blank nodes, so a return for 
 *         one might be [ '_:1' ]
 */
function getRoots(input) {
    var triples = input.match(null, "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", null).toArray();
    var subjects = _.compact(
         _.map(triples, function(triple) { 
    ...
    return  _.filter(subjects, function(subject) { 
        return _.isEmpty(input.match(null, null, subject).toArray());
    });
}
...
function objectValue(object) {
    ...
    if (object.interfaceName === 'NamedNode') {
        ...
        if (object.datatype.toString() === 'http://www.w3.org/2001/XMLSchema#boolean') {
            return (value == 'true');
        }
