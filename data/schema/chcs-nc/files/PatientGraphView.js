define([
    ...
    var PatientGraphView = Backbone.View.extend({
        ...
        initialize: function(params) {
            ...
            this.obscuredDomain = params.obscured;

            if (params && params.dataset && params.dataType && params.dataId) {
                this.dataset = params.dataset;
                this.domain = params.dataType;
                this.patientId = params.dataId;

                var url;
                if (!this.obscuredDomain) {
                    url = '/record?dataset=' + this.dataset + '&id=' + this.patientId + '&datatype=' + this.domain;
                }
        ...
        getJson: function(url) {
            ...
            if (!url) {
                ...
                this.nextroute = 'pgrafTop/' + self.dataset + '/' + self.domain + '/' + self.patientId;
            } else {
                this.nextroute = 'pgraf/' + self.dataset + '/' + self.domain + '/' + self.patientId;
            }
            $.getJSON(url, function(data) {
                ...
                Backbone.history.navigate(self.nextroute);
        ...
        buildTableOfGraphNodes: function (self, graphArray) {
            ...
            for (var i=0; i < graphArray.length; i++) {
                var type, typeLink, linkTag;
                if (graphArray[i].type.indexOf(':') > 0) {
                    type = graphArray[i].type.substring(graphArray[i].type.indexOf(':') + 1);
                } else {
                    type = graphArray[i].type;
                }
                typeLink = '<a href="#schema/' +  self.dataset + '/' + type + '" target="_blank">' + type + '</a>';
                linkTag = '<a data-toggle="collapse" href="#' + graphArray[i]._id
        ...
        buildHiddenTableOfProperties: function(self, node, target, nodeType, level) {
            ...
            for (var key in node) {
                ...
                if (node.hasOwnProperty(key) &&  key !== 'type' && key !== '_id' && key != 'label') {
                    ...
                    if ($.isArray(nodeObject)) {
                        ...
                        for (var i=0; i < nodeObject.length; i++) {
                            var type;
                            if (nodeObject[i].type) {
                                type = nodeObject[i].type.substring(6);
                            } else if (nodeObject[i].id) {
                                type = nodeObject[i].id.match(/^[0-9]*_?[0-9]*-/);
                            ...
                            } else if (typeof nodeObject[i] === 'string') {
                                type = nodeObject[i];
                            } else if (nodeObject instanceof Array) {
                                type = '[ARRAY]';
                            } else {
                                type = '[OBJECT]';
                            }
                        ...
                        var idOrType= self.buildIdOrTypeLinks(self, nodeObject.id, nodeObject.label);
                    ...
                    embeddedClassTable+= '</tr>';
                } else if (key === "id" && Object.keys(node).length === 2) { // we need to check for the case of the singleton multiple
                    //idOrType = node.id;
                    idOrType = self.buildIdOrTypeLinks(self, node.id);
                    labelOrValue = node.label;

                    embeddedClassTable += '<tr>';
                    embeddedClassTable += '<td colspan="2">[SINGLETON MULTIPLE]</td>'
                    embeddedClassTable += '<td>' + labelOrValue + '</td> <td>' + idOrType + '</td>';
                    embeddedClassTable+= '</tr>';
        ...
        buildIdOrTypeLinks: function(self, id) {
            var returnString;
            var idType = id.match(/^[0-9]*_?[0-9]*-/);
            if (idType && idType.length > 0) {
                idType = idType[0];
                idType = idType.substring(0, idType.length - 1);
                returnString = '<a class="record-update-link pointer-to-class" data-id-type="' + idType + '" data-id="' + id + '" data-dataset="' + self.dataset + '"> '
                    + id + '</a>'
                    + '&nbsp;<a href="#schema/' +  self.dataset + '/' + idType + '" target="_blank">(' + idType + ')</a>';
            } else {
                returnString = id;
            }
        ...
        updateRecord: function(event) {
            ...
            this.dataset = event.target.attributes["data-dataset"].value;
