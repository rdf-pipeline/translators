...
define([
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        routes: {
            ...
            'pgrafTop/*pgrafParams': 'pgrafTop',
            'pgraf/*pgrafParams': 'pgraf',
            'pgraf': 'pgraf',
            'rest': 'restClient',
        ...
        pgrafTop: function(pgrafParams) {
            this.pgraf(pgrafParams, true);
        },

        pgraf: function(pgrafParams, obscured) {
            var paramsArray;
            var dataset, dataType, id;

            if (pgrafParams) {
                paramsArray = pgrafParams.split("/");
            }

            if (paramsArray && paramsArray.length > 2) {
                dataset = paramsArray[0];
                dataType = paramsArray[1];
                id = paramsArray[2];
            }

            function setSchemaIdFromJson(dataset, schemaId) {
                Backbone.history.navigate('pgraf/' + dataset + '/' + dataType + '/' + id, {trigger: "true"});
            }
